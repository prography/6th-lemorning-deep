...
class Model():
    def __init__(self):
        self.path_model = './musicnn/MTT_musicnn/'
        self.n_frame_seconds = 3
        self.duration_load_audio = 6
        self.n_frames = librosa.time_to_frames(self.n_frame_seconds, sr=config.SR, n_fft=config.FFT_SIZE, hop_length=config.FFT_HOP) + 1
    ...
    def load_model(self):
        ...
        with tf.name_scope('model'):
            ...
            y, _, _, _, _, _, self.mean_pool, _, _ = models.define_model(self.x, self.is_training, 'MTT_musicnn', num_classes)
    ...
    def extract_features_and_tags(self, audio_file, topN=5):
        ...
        for i in range(batch.shape[0]):
            feat, y = self.sess.run([self.mean_pool, self.normalized_y], feed_dict={self.x: batch[i:i+1], self.is_training:False})
            feats.append(feat)
            ys.append(y)
