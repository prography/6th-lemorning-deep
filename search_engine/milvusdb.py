...
#################################################
    ...
    def check_exist_data_by_key(self, key):
        ...
        _, vector = self.engine.get_entity_by_id(collection_name=self.collection_name, ids=key)
...
#################################################
    ...
    def insert_data(self, key, value):
        ...
        self.engine.insert(collection_name=self.collection_name, records=value, ids=key)
...
#################################################
    ...
    def search_by_feature(self, feature, top_k):
        ...
        feature = self.convert_key_format(feature)

        _, result = self.engine.search(collection_name=self.collection_name, query_records=feature, top_k=top_k)

        li_id = [list(map(lambda x: x.id, result[0])) for i in range(len(result))]
        li_dist = [list(map(lambda x: x.distance, result[0])) for i in range(len(result))]
    ...
    def search_by_key(self, key, top_k):
        ...
        _, vector = self.engine.get_entity_by_id(collection_name=self.collection_name, ids=key)
        _, result = self.engine.search(collection_name=self.collection_name, query_records=vector, top_k=top_k+1)

        li_id = [list(map(lambda x: x.id, result[0][1:])) for i in range(len(result))]
        li_dist = [list(map(lambda x: x.distance, result[0][1:])) for i in range(len(result))]
